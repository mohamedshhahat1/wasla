...
class GoogleOAuthClient:
    ...
    async def exchange(self, *, code: str, code_verifier: str) -> str:
        ...
        :raises GoogleExchangeError: for every failure, with no detail.
        """
        if not code or len(code) > MAX_CODE_LENGTH:
            raise GoogleExchangeError("The authorization code was not usable.")
        ...
        if not isinstance(id_token, str) or not id_token:
            ...
            raise GoogleExchangeError("Google did not return an identity token.")
        return id_token
        ...
        try:
            async with (
                ...
                if response.status_code >= httpx.codes.BAD_REQUEST:
                    ...
                    raise GoogleExchangeError("Google refused the authorization code.")

                async for chunk in response.aiter_bytes():
                    body.extend(chunk)
                    if len(body) > MAX_TOKEN_RESPONSE_BYTES:
                        raise GoogleExchangeError("Google's response was too large.")
        except GoogleExchangeError:
            raise
            ...
            raise GoogleExchangeError("Google could not be reached.") from exc

        try:
            payload: Any = json.loads(bytes(body))
        except json.JSONDecodeError as exc:
            raise GoogleExchangeError("Google's response was not JSON.") from exc
        if not isinstance(payload, dict):
            raise GoogleExchangeError("Google's response was not an object.")
        return payload
