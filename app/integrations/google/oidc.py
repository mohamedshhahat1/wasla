...
ID_TOKEN_ALGORITHM: Final = "RS256"
...
class GoogleKeysUnavailable(Exception):
    ...
    Distinct from :class:`GoogleTokenInvalid` because the two mean opposite
...
class GoogleKeyRing:
    ...
    async def key_for(self, kid: str) -> jwt.PyJWK:
        ...
        if key is None:
            ...
            raise GoogleTokenInvalid("unknown_key_id")
        return key
    ...
    async def _refresh(self) -> None:
        ...
        try:
            ...
            raise GoogleKeysUnavailable("Google published no usable signing keys.") from exc
        ...
        if not keys:
            ...
            raise GoogleKeysUnavailable("Google published no identified signing keys.")
    ...
    async def _fetch(self) -> dict[str, Any]:
        ...
        try:
            async with (
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    if len(body) > MAX_JWKS_BYTES:
                        ...
                        raise GoogleKeysUnavailable("Google's key document was too large.")
        except GoogleKeysUnavailable:
            raise
            ...
            raise GoogleKeysUnavailable("Google's signing keys could not be fetched.") from exc

        try:
            payload = json.loads(bytes(body))
        except json.JSONDecodeError as exc:
            raise GoogleKeysUnavailable("Google's key document was not JSON.") from exc
        if not isinstance(payload, dict):
            raise GoogleKeysUnavailable("Google's key document was not an object.")
        return payload
...
class GoogleIdTokenVerifier:
    ...
    async def verify(self, *, id_token: str, nonce: str) -> GoogleIdentityClaims:
        ...
        :raises GoogleTokenInvalid: the token is not acceptable, for any reason.
        :raises GoogleKeysUnavailable: nothing could be verified either way.
        """
        ...
        if header.get("alg") != ID_TOKEN_ALGORITHM:
            raise GoogleTokenInvalid("unexpected_algorithm")

        kid = header.get("kid")
        if not isinstance(kid, str) or not kid:
            raise GoogleTokenInvalid("missing_key_id")
    ...
    def _header(id_token: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise GoogleTokenInvalid("malformed") from exc
    ...
    def _decode(self, id_token: str, *, key: jwt.PyJWK) -> dict[str, Any]:
        ...
        try:
            ...
            raise GoogleTokenInvalid("expired") from exc
        except InvalidAudienceError as exc:
            raise GoogleTokenInvalid("wrong_audience") from exc
        except InvalidSignatureError as exc:
            raise GoogleTokenInvalid("bad_signature") from exc
        except MissingRequiredClaimError as exc:
            raise GoogleTokenInvalid("missing_claim") from exc
        except InvalidTokenError as exc:
    ...
    def _check_nonce(payload: dict[str, Any], *, expected: str) -> None:
        ...
        if not isinstance(presented, str) or not presented:
            raise GoogleTokenInvalid("missing_nonce")
        if not hmac.compare_digest(presented, expected):
            raise GoogleTokenInvalid("wrong_nonce")
    ...
    def _extract(payload: dict[str, Any]) -> GoogleIdentityClaims:
        ...
        if not isinstance(subject, str) or not subject.strip():
            raise GoogleTokenInvalid("missing_subject")
        ...
        if not isinstance(email, str) or not email.strip():
            ...
            raise GoogleTokenInvalid("missing_email")
