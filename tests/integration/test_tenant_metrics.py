...
def _metrics(session: AsyncSession, tenant: Tenant) -> TenantMetricsRepository:
...
async def test_traffic_is_counted_in_both_directions(db_session: AsyncSession) -> None:
...
async def test_a_burst_of_customer_messages_is_one_wait(db_session: AsyncSession) -> None:
...
async def test_two_separate_waits_are_averaged(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
    conversation = await _conversation(db_session, tenant)
    db_session.add_all(
        [
...
async def test_a_customer_still_waiting_is_counted_not_averaged(db_session: AsyncSession) -> None:
...
async def test_a_failed_reply_is_not_a_reply(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
    conversation = await _conversation(db_session, tenant)
    db_session.add_all(
        [
...
async def test_a_conversation_nobody_took_over_counts_as_resolved(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
...
async def test_one_conversation_handed_over_twice_is_still_one(db_session: AsyncSession) -> None:
...
async def test_a_workspace_with_no_traffic_reports_zero_not_an_error(
...
async def test_pipeline_counts_come_from_the_leads_themselves(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
    db_session.add_all(
        [
            Lead(
...
async def test_an_unhappy_customer_is_counted_once_however_often_they_complain(
    db_session: AsyncSession,
) -> None:
    tenant = await _tenant(db_session)
    conversation = await _conversation(db_session, tenant)
...
async def test_delivery_is_read_from_the_message_not_the_recipient(
    db_session: AsyncSession,
) -> None:
...
async def test_a_row_on_the_upper_bound_belongs_to_the_next_window(
    db_session: AsyncSession,
) -> None:
    tenant = await _tenant(db_session)
...
async def test_a_report_never_reaches_another_workspace(db_session: AsyncSession) -> None:
...
async def test_the_report_carries_the_window_it_applied(db_session: AsyncSession) -> None:
