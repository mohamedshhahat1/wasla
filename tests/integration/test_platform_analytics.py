...
async def test_the_overview_counts_every_workspace_and_its_numbers(
...
async def test_platform_usage_is_the_sum_of_every_workspace(db_session: AsyncSession) -> None:
...
async def test_each_workspace_carries_its_own_consumption(db_session: AsyncSession) -> None:
    acme = await _tenant(db_session, "acme")
    rival = await _tenant(db_session, "rival")
...
async def test_workspaces_can_be_searched_by_name_or_address(db_session: AsyncSession) -> None:
...
async def test_a_wildcard_in_a_search_is_not_a_wildcard(db_session: AsyncSession) -> None:
...
async def test_workspaces_can_be_filtered_by_status(db_session: AsyncSession) -> None:
    await _tenant(db_session, "acme")
    await _tenant(db_session, "paused", status=TenantStatus.SUSPENDED)
    await db_session.flush()
...
async def test_the_page_total_counts_matches_not_the_page(db_session: AsyncSession) -> None:
...
async def test_per_tenant_usage_of_an_empty_page_asks_nothing(db_session: AsyncSession) -> None:
...
class StubPlatform:
    ...
    async def workspaces(self, **kwargs: Any) -> None:
        self.calls += 1
        raise AssertionError("The guard should have refused this request.")


def _as_user(app: FastAPI, role: PlatformRole | None) -> None:
...
@pytest.fixture
def platform(app: FastAPI) -> StubPlatform:
    stub = StubPlatform()
    app.dependency_overrides[get_platform_analytics_service] = lambda: stub
    return stub


async def test_a_workspace_owner_is_refused(
    client: AsyncClient, app: FastAPI, platform: StubPlatform
) -> None:
...
class AnsweringPlatform:
    ...
    async def workspaces(
        self, *, since: datetime | None = None, until: datetime | None = None, **kwargs: Any
    ) -> WorkspacePage:
        window = resolve_window(since=since, until=until)
...
@pytest.fixture
def answering(app: FastAPI) -> AnsweringPlatform:
    stub = AnsweringPlatform()
    app.dependency_overrides[get_platform_analytics_service] = lambda: stub
    return stub


async def test_platform_staff_can_read_the_overview(
    client: AsyncClient, app: FastAPI, answering: AnsweringPlatform
) -> None:
...
async def test_platform_staff_can_list_workspaces_with_their_usage(
    client: AsyncClient, app: FastAPI, answering: AnsweringPlatform
) -> None:
