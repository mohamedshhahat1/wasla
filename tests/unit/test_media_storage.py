...
@pytest.fixture
def storage(tmp_path) -> LocalMediaStorage:
    return LocalMediaStorage(tmp_path)


def test_a_key_starts_with_the_workspace():
...
def test_a_key_is_generated_not_derived():
...
def test_a_known_type_gets_its_extension():
...
def test_an_unknown_type_gets_no_extension_rather_than_a_guess():
    key = build_key(tenant_id=TENANT, mime_type="application/x-invented")
    assert key.rsplit("/", 1)[-1].count(".") == 0


def test_every_declared_extension_is_lowercase_and_dotted():
...
async def test_deleting_is_idempotent(storage):
    key = await storage.put(tenant_id=TENANT, data=b"hello")
...
async def test_reading_a_key_that_was_never_stored_fails_cleanly(storage):
...
async def test_a_key_that_escapes_the_root_is_refused(storage, key):
...
async def test_an_escaping_key_cannot_be_deleted_either(storage):
    with pytest.raises(StorageError):
        await storage.delete("../../../etc/passwd")


async def test_two_workspaces_files_do_not_share_a_prefix(storage):
