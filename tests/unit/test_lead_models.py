...
def test_a_lead_moves_forward_through_the_pipeline(start: LeadStatus, target: LeadStatus) -> None:
...
def test_illegal_moves_are_refused(start: LeadStatus, target: LeadStatus) -> None:
...
def test_setting_the_status_a_lead_already_has_is_allowed(status: LeadStatus) -> None:
...
def test_terminal_statuses_lead_nowhere_except_reopening() -> None:
    for status in TERMINAL_STATUSES:
        assert ALLOWED_TRANSITIONS[status] <= {LeadStatus.NEW}


def test_lost_is_reachable_from_every_open_status() -> None:
...
def test_a_score_is_pinned_to_its_bounds(value: int, expected: int) -> None:
    """Clamped rather than rejected: a model's stray number should not lose the lead."""
    assert clamp_score(value) == expected


def test_an_agent_cannot_write_judgement_fields() -> None:
...
def test_an_agent_can_write_the_details_a_customer_states() -> None:
    assert {"name", "email", "phone", "interest", "budget_amount"} <= AGENT_WRITABLE_FIELDS


def test_agent_writable_fields_all_exist_on_the_lead() -> None:
...
def test_a_lead_reports_whether_it_is_closed() -> None:
