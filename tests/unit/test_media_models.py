...
def test_an_unread_file_holds_up_the_reply():
...
def test_a_read_file_no_longer_holds_up_the_reply():
    assert _media(status=MediaStatus.READY).is_resolved is True


def test_an_unreadable_file_does_not_hold_up_the_reply_forever():
...
def test_the_table_is_tenant_scoped():
    ...
    assert "tenant_id" in MessageMedia.__table__.columns


def test_one_file_per_message():
    """A webhook replay must not be able to add a second row for one message."""
    constraints = {
        constraint.name
        for constraint in MessageMedia.__table__.constraints
