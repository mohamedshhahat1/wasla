...
def test_a_well_formed_message_is_accepted() -> None:
    message = _message()
...
def test_a_control_character_in_a_recipient_is_refused(injection: str) -> None:
    with pytest.raises(ValueError):
        _message(to=(f"person@example.com{injection}Bcc: attacker@evil.test",))


@pytest.mark.parametrize("injection", ["\r", "\n", "\r\n", "\x00"])
def test_a_control_character_in_the_sender_is_refused(injection: str) -> None:
    with pytest.raises(ValueError):
        _message(sender=f"no-reply@example.com{injection}Bcc: attacker@evil.test")


@pytest.mark.parametrize("injection", ["\r", "\n", "\r\n", "\x00"])
def test_a_control_character_in_the_subject_is_refused(injection: str) -> None:
    with pytest.raises(ValueError):
        _message(subject=f"Hello{injection}Bcc: attacker@evil.test")


@pytest.mark.parametrize("injection", ["\r", "\n", "\r\n", "\x00"])
def test_a_control_character_in_the_reply_to_is_refused(injection: str) -> None:
    with pytest.raises(ValueError):
        _message(reply_to=f"reply@example.com{injection}X-Evil: 1")


def test_a_message_with_no_recipients_is_refused() -> None:
    with pytest.raises(ValueError):
        _message(to=())


def test_more_recipients_than_the_cap_are_refused() -> None:
...
def test_exactly_the_recipient_cap_is_allowed() -> None:
...
def test_an_oversized_subject_is_refused() -> None:
    with pytest.raises(ValueError):
...
def test_an_empty_text_body_is_refused() -> None:
    """A message with nothing in it is a bug upstream, not a message."""
    with pytest.raises(ValueError):
        _message(text="   ")


def test_an_oversized_text_body_is_refused() -> None:
    with pytest.raises(ValueError):
        _message(text="b" * (MAX_BODY_BYTES + 1))


def test_an_oversized_html_body_is_refused() -> None:
    with pytest.raises(ValueError):
        _message(html="<p>" + "b" * (MAX_BODY_BYTES + 1) + "</p>")


def test_the_body_cap_counts_bytes_rather_than_characters() -> None:
    """A multi-byte character must not buy four times the payload."""
    with pytest.raises(ValueError):
        _message(text="一" * MAX_BODY_BYTES)


def test_an_oversized_address_is_refused() -> None:
    with pytest.raises(ValueError):
...
def test_surrounding_whitespace_is_stripped_from_an_address() -> None:
    assert validate_address("  person@example.com  ") == "person@example.com"


def test_a_message_exposes_no_way_to_set_a_header() -> None:
