...
async def _reject(verifier, token, *, nonce=NONCE):
    with pytest.raises(GoogleTokenInvalid) as caught:
...
async def test_a_rotated_key_is_picked_up():
    """Google rotates without notice; a cache miss must not be a failure."""
    ring = _FixedKeyRing(
        _jwks((KID, _SIGNING_KEY)),
        _jwks((KID, _SIGNING_KEY), (ROTATED_KID, _ATTACKER_KEY)),
    )
    ...
    rotated = _sign(_claims(), key=_ATTACKER_KEY, kid=ROTATED_KID)
...
async def test_nothing_can_be_verified_when_the_keys_were_never_fetched():
    verifier, _ = _verifier(GoogleKeysUnavailable("down"))
    with pytest.raises(GoogleKeysUnavailable):
        await verifier.verify(id_token=_sign(_claims()), nonce=NONCE)
...
def _install(monkeypatch, stream):
    monkeypatch.setattr(
        ...
        lambda *, timeout: _FakeHttpClient(stream),  # noqa: ARG005
    )
