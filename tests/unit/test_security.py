...
def test_an_unreadable_stored_hash_fails_the_login_without_raising():
...
def test_timing_equaliser_is_safe_to_call_with_no_account():
...
def test_refresh_token_carries_no_workspace(settings):
...
def test_each_token_has_its_own_identifier(settings):
...
def test_an_access_token_is_not_a_refresh_token(settings):
...
def test_expired_tokens_are_rejected(settings):
...
def test_tampered_tokens_are_rejected(settings):
    token, _ = create_access_token(settings=settings, subject=SUBJECT)
...
def test_invitation_token_is_stored_only_as_a_hash():
...
def test_invitation_tokens_do_not_repeat():
