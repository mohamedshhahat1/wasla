...
def _template(**overrides) -> WhatsAppTemplate:
    values = {
        "name": "order_update",
        "language": "ar_EG",
...
def test_metas_statuses_map_onto_ours(meta_status, expected):
    assert META_STATUSES[meta_status] is expected


def test_a_status_meta_invents_later_is_not_approved():
...
def test_metas_earlier_category_names_still_resolve():
...
def test_only_approved_is_sendable():
...
def test_only_marketing_templates_are_marketing():
...
def test_a_rejected_template_is_refused():
