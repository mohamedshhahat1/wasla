...
def test_a_signature_produced_by_the_app_secret_verifies() -> None:
    header = _signature(APP_SECRET, BODY)

    assert verify_signature(payload=BODY, header=header, app_secret=APP_SECRET)


def test_the_computed_header_matches_the_documented_format() -> None:
...
def test_a_missing_header_is_a_failure_not_a_pass() -> None:
...
def test_a_header_without_the_prefix_does_not_verify() -> None:
