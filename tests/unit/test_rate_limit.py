...
def _limiter(**kwargs) -> tuple[RateLimiter, FakeCommands]:
    commands = FakeCommands(**kwargs)
    return RateLimiter(FakeRedis(commands)), commands
...
async def test_a_refused_request_still_counts():
...
async def test_the_window_is_set_once_and_not_refreshed():
...
async def test_a_key_with_no_expiry_is_given_one():
...
async def test_two_callers_are_counted_separately():
    limiter, _ = _limiter()
    for _ in range(3):
        await limiter.check(POLICY, "1.2.3.4")
...
async def test_two_policies_over_one_identity_are_counted_separately():
...
async def test_enforcing_raises_with_a_retry_after_header():
...
async def test_an_allowed_response_carries_no_retry_after():
