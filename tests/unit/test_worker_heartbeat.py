...
def _redis(**kwargs: Any) -> tuple[FakeRedis, FakeCommands]:
    commands = FakeCommands(**kwargs)
    return FakeRedis(commands), commands


async def test_a_beat_expires_on_its_own() -> None:
    ...
    await Heartbeat(as_redis_client(redis), kind="agent").beat()

    assert commands.keys[heartbeat_key("agent")] == DEFAULT_TTL_SECONDS


async def test_a_kind_that_has_beaten_is_alive() -> None:
    redis, _ = _redis()
    heartbeat = Heartbeat(as_redis_client(redis), kind="agent")
...
async def test_a_kind_that_has_not_beaten_is_not_alive() -> None:
    redis, _ = _redis()

    assert await Heartbeat(as_redis_client(redis), kind="agent").is_alive() is False


async def test_a_failed_beat_does_not_kill_the_worker() -> None:
    ...
    assert await Heartbeat(as_redis_client(redis), kind="agent").beat() is False


async def test_an_unreadable_heartbeat_reports_not_alive() -> None:
...
async def test_every_configured_loop_must_be_beating() -> None:
    ...
    redis, _ = _redis()
    await Heartbeat(as_redis_client(redis), kind="agent").beat()
    await Heartbeat(as_redis_client(redis), kind="media").beat()

    assert await all_alive(as_redis_client(redis), ("agent", "media")) is True
    assert await all_alive(as_redis_client(redis), ("agent", "media", "campaign")) is False


async def test_a_worker_running_nothing_is_not_healthy() -> None:
    """An empty set of kinds is a misconfiguration, not a healthy idle."""
    redis, _ = _redis()

    assert await all_alive(as_redis_client(redis), ()) is False


async def test_kinds_are_counted_separately() -> None:
    redis, _ = _redis()
    await Heartbeat(as_redis_client(redis), kind="agent").beat()

    assert await Heartbeat(as_redis_client(redis), kind="campaign").is_alive() is False


# ------------------------------------------------------------- the beat task


async def test_the_beat_task_refreshes_until_asked_to_stop() -> None:
    redis, commands = _redis()
    stopping = asyncio.Event()

    task = asyncio.create_task(
        beat_while_running(
            as_redis_client(redis), ("agent", "media"), stopping=stopping, interval_seconds=0.01
        )
    )
...
async def test_the_beat_task_stops_promptly() -> None:
    ...
    task = asyncio.create_task(
        beat_while_running(
            as_redis_client(redis), ("agent",), stopping=stopping, interval_seconds=30
        )
    )
...
async def test_the_beat_task_survives_a_redis_outage() -> None:
    ...
    task = asyncio.create_task(
        beat_while_running(
            as_redis_client(redis), ("agent",), stopping=stopping, interval_seconds=0.01
        )
    )
    await asyncio.sleep(0.05)
    stopping.set()
...
def test_every_worker_kind_has_a_key(kind: str) -> None:
