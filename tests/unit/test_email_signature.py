...
def test_a_delivery_signed_with_the_secret_verifies() -> None:
    assert _verify()


def test_the_computed_signature_matches_the_documented_format() -> None:
...
def test_the_signed_content_is_id_timestamp_and_body_not_the_body_alone() -> None:
...
def test_the_hmac_key_is_the_decoded_secret_not_the_prefixed_string() -> None:
...
def test_a_signature_cannot_be_replayed_under_a_different_message_id() -> None:
    # The header is genuine, for a delivery this is not.
    assert not _verify(message_id="msg_someone_elses")


def test_a_signature_cannot_be_replayed_under_a_different_timestamp() -> None:
    fresh = str(int(NOW) + 10)

    assert not _verify(timestamp=fresh)


def test_a_stale_delivery_does_not_verify() -> None:
...
def test_a_delivery_from_the_future_does_not_verify() -> None:
    # A forged timestamp would otherwise buy a much longer replay window.
    assert not _verify(now=NOW - DEFAULT_TOLERANCE_SECONDS - 1)


def test_a_delivery_inside_the_window_still_verifies() -> None:
    assert _verify(now=NOW + DEFAULT_TOLERANCE_SECONDS)
    assert _verify(now=NOW - DEFAULT_TOLERANCE_SECONDS)


def test_an_unparseable_timestamp_does_not_verify() -> None:
    stamp = "not-a-timestamp"
    assert not _verify(timestamp=stamp, signature_header=_sign(timestamp=stamp))


def test_missing_headers_are_a_failure_not_a_pass() -> None:
...
def test_a_malformed_secret_never_verifies() -> None:
...
def test_one_valid_entry_among_several_verifies() -> None:
...
def test_an_unknown_signature_version_is_skipped_not_trusted() -> None:
...
def test_a_garbled_header_does_not_verify() -> None:
