...
def test_severity_runs_from_pleased_to_furious():
    ordered = [
...
def test_a_threshold_matches_itself_and_anything_worse():
...
def test_a_threshold_does_not_match_something_milder():
...
def test_an_unhappy_customer_raises_the_priority(label, expected):
    assert raised_priority(ConversationPriority.NORMAL, label) is expected


def test_a_happy_customer_does_not_touch_the_priority():
...
def test_priority_is_never_lowered_by_a_milder_reading():
