...
- [ ] Tenant administration from the platform layer — creating, suspending or deleting a workspace. The reason has changed since this was written: the audit trail those actions need exists (ADR-033), and the platform layer now uses it for invoice administration and account enable/disable. What is still missing is a product answer for what happens to a suspended workspace's in-flight conversations, which is a decision rather than a build
