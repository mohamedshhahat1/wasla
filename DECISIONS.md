...
## ADR-095 — Platform Staff Reading Across Workspaces Leave a Trail

**Context.** Every platform *write* was audited — a payment recorded, an invoice
voided, an account disabled or restored. No platform *read* was.
`GET /platform/overview` and `GET /platform/tenants` returned cross-workspace
figures and a searchable list of every customer on the estate, and neither
wrote anything down.

That asymmetry was defensible on the reasoning `AuditAction` already states:
reading a page is not audited, because a row per page view buries the acts that
matter under a million that do not. And it stops being defensible at exactly the
point a customer asks *who looked at our workspace* — a question the trail was
built to answer and could not.

**Decision. Audit the reads on this router, and no other read in the API.**

What makes these different is not that they are reads. It is whose data they
are. A workspace administrator reading their own inbox is looking at their own
business. A platform administrator reading the estate is looking at somebody
else's, and there are perhaps a handful of such people making a handful of
requests a day — so the volume argument that keeps ordinary reads out of this
table simply does not reach them.

| Route | Sensitive? | Action | Target workspace | Stored in the entry |
| --- | --- | --- | --- | --- |
| `GET /platform/overview` | Aggregate only | `platform_overview_read` | none | resource, whether a window was given |
| `GET /platform/tenants` | **Names customers and their traffic** | `platform_workspaces_read` | none | resource, count returned, whether searched, whether filtered |
| `GET /platform/audit-logs` | **Another workspace's whole trail** | `platform_audit_log_read` | when narrowed to one | resource, count returned |

**The aggregate route is audited too, and that is a deliberate answer to a
question the audit left open.** The reads there name no workspace, so nothing
about a particular customer is disclosed — but "who was looking at us at all" is
part of what a privacy trail answers, and six requests a day costs nothing.
What is *not* recorded is the window boundary: a date range is a filter, and
filters are how an operator narrows to one customer.

**The entry names the class of data, never the data.** No search term — somebody
looking for one business types an address as readily as a company name, so
recording it would make the trail a copy of the thing it exists to police. No
workspace name, no filter values, no customer content. A count and a boolean say
how much was seen and whether it was narrowed, which is what an investigation
needs.

**A workspace id is the subject of an entry rather than its payload.** It is
recorded on the audit-log read, because "somebody read *your* trail" is
precisely the access a customer is entitled to be told about, and an entry that
declined to say which workspace would be unusable for the purpose it exists for.

**It fails closed by inheriting rather than by choosing.** The entry is staged in
the request's own session and `CommittingRoute` commits that transaction before
the response is emitted (ADR-062), so a failure to record the access is a
failure to serve it. That is the rule `AuditTrail` already states for writes —
"if we cannot say who disconnected the number, we do not disconnect it" — and it
needed no new machinery to hold here. Nothing swallows.

**The entry is written after the read, not in a dependency.** A request the guard
turned away is not an access. Recording in a dependency would have made every
403 look like a successful read, and a trail that cannot tell "somebody read
this" from "somebody tried" is one nobody can act on.

**The existing `audit_logs` table, not a second one.** It already carries a
nullable `tenant_id` for platform acts, an actor kind that distinguishes
platform staff, a bounded action vocabulary and a `meta` column. A separate
read-log would have split the answer to "what did this person do" across two
tables, which is the shape of an investigation nobody wants to run.

**Consequences.**

The audit log now contains entries generated by the people reading it, which was
already true of platform writes and is the point: `/platform/audit-logs` shows
its own reads, and the platform owner is not exempt.

Three actions rather than one, because they are three different amounts of
access and an investigation filters on which. The vocabulary stays bounded and
these are metric-safe names carrying no identifier.

**What is proved rather than argued.**
`tests/integration/test_platform_access_audit.py` drives the real routes with
the real guard against a real database: it asserts an entry per read, that the
workspace is named where one was, that a canary search term and a canary
workspace name reach no column and no `meta` field, that a member with no
platform role produces none, and that a refused request leaves nothing behind.
