...
The transaction belongs to the worker, which is the one component here with no request to borrow a session from. It opens one session per job, and the outbound message row and the conversation's timestamps land together or not at all.

**The provider call happens with no connection held** (ADR-080). The orchestrator commits and returns the connection to the pool before every inference and before every per-round meter write, so the number of turns a worker can run at once is the depth of the queue rather than `pool_size + max_overflow`. A turn is therefore not one transaction: each round commits what the previous round's tools finished, which is the better direction for a queue that does not retry after the provider is engaged. And because a commit ends a snapshot, the workspace's status and deletion, the agent's status, the conversation's mode and status and the number are deliberately read again, as column values, before a reply is sent — a colleague who took the conversation over, closed it or disabled the agent while the model was composing, or an operator who suspended the workspace, gets no AI answer arriving underneath them (ADR-105). The WhatsApp send stays inside a transaction, which is a different boundary: the message row is flushed before the call so an attempt exists as a row whatever happens next (ADR-074).
...
Argon2id password hashing with rehash-on-login, typed access and refresh tokens, rotating refresh tokens with a Redis denylist, a live-current-user dependency, verified-user gates for workspace/platform business actions, workspace resolution and switching from the token, and role dependencies for both scopes. Soft-deleted identities are tombstones: ordinary lookup excludes them and deletion atomically disables the row and raises its token version. Access tokens are intentionally not individually revocable and membership is re-verified per request; the reasoning for both, and the invitation flow, is in [docs/AUTH.md](docs/AUTH.md).

**Every access token is minted by one function**, `AuthService._access_token` (ADR-058). Login, registration, refresh, a federated session and a workspace switch differ in what they pass it — whether a workspace is selected, and whether a refresh token accompanies it — never in how the token is built. A second issuance site is a second policy, and the one that existed omitted `token_version`, so switching workspace returned a token every later request refused as revoked.
...
**"Claims" in that diagram is literal** (ADR-082). Every phase of the sweep takes its rows with `FOR UPDATE ... SKIP LOCKED` and commits per claim, so two billing workers divide a cohort rather than racing to bill it — the unique constraint above is the floor, not the mechanism, and it used to fire *after* the work rather than instead of it. `CLAIM_LIMIT` is a batch size and a phase drains until it finds nothing, so a first-of-the-month cohort finishes in one pass instead of at 200 every ten minutes.
...
| `ai_turn` | The agent worker, in the transaction that engages the turn | What a plan's AI allowance counts (ADR-104). Charged once per turn whatever it cost in provider calls, never by a duplicate job that lost the claim, and never for a turn the AI will not take |
| `ai_request` | The agent worker, one row before each inference round | Provider cost, not entitlement: nothing checks it against a limit. `meta.purpose = agent`. A crash between recording and calling records a request that did not happen, which is the cheaper mistake |
| `ai_input_token`, `ai_output_token` | The agent worker, once the inference is done, committed before any send is attempted | A reply that could not be sent still used the tokens, so they are durable before the send can fail |
| `ai_request` (again) | The sentiment service | The classification is a provider call of its own, on its own model, `meta.purpose = sentiment`. The caller never sees it, so the caller cannot count it — and it never spends the customer's turn |
