...
| `BACKUP_S3_SSE` | **Required once `BACKUP_DESTINATION=s3`.** `AES256` or `aws:kms`. The uploader refuses to send a dump without it and refuses to record a success unless the store confirms the object is encrypted. A dump is every conversation, phone number and lead the platform holds (ADR-090) |
