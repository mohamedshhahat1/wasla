...
- **Human ownership outranks a stale AI decision.** A colleague who took the conversation over during the inference is the owner: the lead tool refuses, the follow-up tool refuses, and the handoff tool is a no-op that leaves the colleague's own reason and writes no agent handoff row (TOOL-06, TOOL-07). That holds at the write, not only at the tool's read: the handoff goes through `InboxService.hand_off`, which moves the conversation only if it is still the AI's when the row is locked, so a takeover committed between the tool's check and its write wins, the call is refused, and the turn ends `suppressed_human` without another inference (CRM-02, ADR-111). Sentiment escalation and the empty-response and quota fallbacks use the same transition. An agent's handoff never assigns anybody; a colleague's takeover makes them the owner ([CRM.md](CRM.md)). A handoff that *does* succeed stops the rest of its own response, so `[handoff, schedule_follow_up]` cannot plant a nudge on a conversation just handed over (PD-TOOLS-02).
