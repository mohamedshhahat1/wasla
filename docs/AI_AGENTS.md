...
- **A workspace no longer served.** A suspended workspace, and a deleted one for the whole of its retention window, gets no inference, no tool and no message. Retention decides when data is erased; it does not keep the AI serving in the meantime. The guarantee holds *during* a turn as well as before it: the executor re-reads the workspace, the agent, the conversation and the number immediately before every tool call, so a workspace suspended while a model was composing writes no CRM record and schedules no customer message (TOOL-03). A follow-up a tool scheduled earlier is cancelled at the transition and refused again at send time, so nothing automated leaves a workspace that is not being served (TOOL-04).
